...
foreach ($articles as $x){
    ...
    if ($_SESSION['username'] != ""){
            ...
            if ($user['username'] == $_SESSION['username']){
                foreach($user['favorites'] as $favorite){
                    if ($favorite['title'] == $x->title){
                        echo("<div class='col-md-6'><button class='rounded btn-primary float-right' id='".cleanString($x->title)."-button' onclick=\"unfavorite('".cleanString($x->title)."')\">Unfavorite</button></div>");
                        $fav = True;
                        break;
                    }
                }    
        ...
        if ($fav==False){
            echo("<div class='col-md-6'><button class='rounded btn-primary float-right' id='".cleanString($x->title)."-button' onclick=\"favorite('".cleanString($x->title)."')\">Favorite</button></div>");
        }
